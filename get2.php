...
$sql_ext = "SELECT * FROM external2_marks WHERE student_id = '$student_id' AND semester = '$semester'";
$result_ext = $conn->query($sql_ext);

// Retrieve internal marks
$sql_int = "SELECT * FROM internal2_marks WHERE student_id = '$student_id' AND semester = '$semester'";
...
if ($result_ext->num_rows > 0 && $result_int->num_rows > 0) {
    ...
    for ($i = 1; $i <= 9; $i++) { // Loop for 11 subjects
    ...
    $total_possible_marks = 100 * 9; // Assuming each subject has a maximum of 100 marks (external + internal) and 11 subjects
