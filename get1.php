...
$sql_ext = "SELECT * FROM external1_marks WHERE student_id = '$student_id' AND semester = '$semester'";
$result_ext = $conn->query($sql_ext);

// Retrieve internal marks
$sql_int = "SELECT * FROM internal1_marks WHERE student_id = '$student_id' AND semester = '$semester'";
...
if ($result_ext->num_rows > 0 && $result_int->num_rows > 0) {
    ...
    for ($i = 1; $i <= 10; $i++) { // Loop for 10 subjects
    ...
    $total_possible_marks = 100 * 10; // Assuming each subject has a maximum of 100 marks (external + internal) and 10 subjects
...
document.getElementById("screenshotBtn").addEventListener("click", function() {
    html2canvas(document.body).then(function(canvas) {
        // Clear previous screenshot if it exists
        const existingScreenshot = document.getElementById('screenshot');
